...
const newBlog = `        <!-- Blog View (Initially Hidden) -->
                ...
                <div class="blog-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 50px;">
                    ...
                    <div class="blog-card blog-card-hover" style="background: transparent;">
                        ...
                        <div class="blog-content">
                            ...
                            <p style="color: #555; font-size: 0.95rem; line-height: 1.8;">건강검진이 끝나고 두꺼운 한국어 결과지를 받았을 때의 당혹감은 말로 표현하기 어렵습니다. "간 기능 수치 ALT 42 U/L", "혈청 크레아티닌 0.9 mg/dL"처럼 숫자는 있지만 그것이 정상인지, 주의가 필요한지 전혀 알 수 없었습니다. 번역 앱으로 돌려봐도 용어 번역에 그칠 뿐, 제 건강에 어떤 의미가 있는지는 알 수 없었습니다.<br><br>CHECKIT은 병원 발행 원본 결과지와 함께, 각 항목의 모국어 번역본 그리고 국제 표준 질병 코드(ICD-10/KCD)를 병기한 참고 문서를 함께 제공했습니다. "간 수치가 경미하게 상승 범위에 있다", "이 항목은 다음 검진 전 추적 관찰이 권장되는 수치"처럼 알기 쉬운 언어로 풀어주어, 본국의 주치의와 후속 상담을 할 때도 훨씬 정확하게 소통할 수 있었습니다.<br><br>단, 이 번역 문서는 참고용이며 원본 결과지를 대체하는 공식 의료 서류가 아님을 명확히 안내받았습니다. 그럼에도 불구하고 내 몸에서 무슨 일이 일어나고 있는지를 처음으로 제대로 이해할 수 있었다는 것, 그것만으로도 CHECKIT의 가치는 충분했습니다.</p>
                        </div>
                    </div>
                </div>
            </div>
...
const startMarker = '        <!-- Blog View (Initially Hidden) -->';
const endMarker = '        <!-- My Page View (Initially Hidden) -->';
...
const result = html.substring(0, startIdx) + newBlog + '\n\n        ' + html.substring(endIdx);
fs.writeFileSync('index.html', result, 'utf8');
console.log('Done! File size: ' + result.length + ' bytes');
