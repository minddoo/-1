...
$newBlog = @'
        ...
        <div id="blog-view" class="hidden-view" style="display: none; background: #fbfaf8; padding-bottom: 120px; font-family: 'Montserrat', sans-serif;">
            ...
            <style>
                ...
                .blog-card-hover:hover h3 { color: #4b6b8e; }
            </style>
            ...
            <div class="blog-hero" style="padding: 120px 0 60px; text-align: center; position: relative;">
                <div class="container">
                    ...
                    <div class="featured-post blog-card-hover" style="position: relative; border-radius: 8px; overflow: hidden; height: 500px; text-align: left; box-shadow: 0 10px 30px rgba(0,0,0,0.05); cursor: pointer;">
                ...
                <div class="blog-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 50px;">
                    <div class="blog-card blog-card-hover" style="background: transparent;">
                        <div style="position: relative; overflow: hidden; border-radius: 6px; margin-bottom: 24px;">
                            <img src="assets/blog/card2.png" alt="Pricing" style="width: 100%; height: 260px; object-fit: cover; transition: transform 0.6s ease;" class="blog-card-img">
                        </div>
                    ...
                    <div class="blog-card blog-card-hover" style="background: transparent;">
                        ...
                        <div class="blog-content">
                            ...
                            <p style="color: #555; font-size: 0.95rem; line-height: 1.8;">한국 건강검진을 처음 예약했을 때, 대장내시경 전 준비 약품의 복용법과 전날 식이 제한 지침이 한국어로만 제공되어 완전히 막막했습니다. 번역 앱을 돌려봤지만 용량에 맞게 물에 희석하여 천천히 복용처럼 모호한 번역은 실질적인 도움이 전혀 되지 않았습니다.<br><br>CHECKIT의 비의료 행정 전담 지원은 달랐습니다. 예약 확정 직후부터 검진 당일까지, 복용 시간표, 식단 제한 항목, 병원 도착 시 제출 서류, 검사 당일 동선까지 모두 제 모국어로 정리된 문서로 받았습니다. 의료적 판단이나 처방은 물론 하지 않지만, 병원을 이용하는 데 필요한 모든 행정 절차를 빠짐없이 안내해주었습니다.<br><br>검진 당일 병원에 도착했을 때, 저는 이미 무엇을 어떻게 해야 하는지 모두 파악하고 있었습니다. 단순히 말이 통하는 것을 넘어, 완전히 준비된 상태로 검진에 임할 수 있었다는 것이 CHECKIT 서비스의 진짜 가치였습니다.</p>
                        </div>
                    </div>
                    <div class="blog-card blog-card-hover" style="background: transparent;">
                        <div style="position: relative; overflow: hidden; border-radius: 6px; margin-bottom: 24px;">
                            <img src="assets/blog/card3.png" alt="Translation" style="width: 100%; height: 260px; object-fit: cover; transition: transform 0.6s ease;" class="blog-card-img">
                        </div>
                        <div class="blog-content">
                            ...
                            <p style="color: #555; font-size: 0.95rem; line-height: 1.8;">건강검진이 끝나고 두꺼운 한국어 결과지를 받았을 때의 당혹감은 말로 표현하기 어렵습니다. 간 기능 수치 ALT 42 U/L, 혈청 크레아티닌 0.9 mg/dL처럼 숫자는 있지만 그것이 정상인지, 주의가 필요한지 전혀 알 수 없었습니다. 번역 앱으로 돌려봐도 용어 번역에 그칠 뿐, 제 건강에 어떤 의미가 있는지는 알 수 없었습니다.<br><br>CHECKIT은 병원 발행 원본 결과지와 함께, 각 항목의 모국어 번역본 그리고 국제 표준 질병 코드(ICD-10/KCD)를 병기한 참고 문서를 함께 제공했습니다. 간 수치가 경미하게 상승 범위에 있다, 이 항목은 다음 검진 전 추적 관찰이 권장되는 수치처럼 알기 쉬운 언어로 풀어주어, 본국의 주치의와 후속 상담을 할 때도 훨씬 정확하게 소통할 수 있었습니다.<br><br>단, 이 번역 문서는 참고용이며 원본 결과지를 대체하는 공식 의료 서류가 아님을 명확히 안내받았습니다. 그럼에도 불구하고 내 몸에서 무슨 일이 일어나고 있는지를 처음으로 제대로 이해할 수 있었다는 것, 그것만으로도 CHECKIT의 가치는 충분했습니다.</p>
                        </div>
                    </div>
                </div>
            </div>
...
$startMarker = '        <!-- Blog View (Initially Hidden) -->'
$endMarker = '        <!-- My Page View (Initially Hidden) -->'
...
$result = $content.Substring(0, $startIdx) + $newBlog + "`r`n`r`n        " + $content.Substring($endIdx)

[System.IO.File]::WriteAllText((Resolve-Path 'index.html').Path, $result, [System.Text.Encoding]::UTF8)
Write-Host "Done! File size: $($result.Length) bytes"
